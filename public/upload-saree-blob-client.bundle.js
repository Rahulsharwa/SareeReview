...
-----END ${descriptor}-----`;
  ...
  // src/upload-saree-blob-client.js
  var PREFIX = "upload-saree/staging";
  var HEIC_DECODER_SRC = "/upload-saree-heic-converter.bundle.js";
  var HEIC_MIME_TYPES = /* @__PURE__ */ new Set(["image/heic", "image/heif"]);
  var HEIC_EXTENSIONS = /* @__PURE__ */ new Set(["heic", "heif"]);
  var heicModulePromise = null;
  ...
  function createHeicError(message2, code, details = {}) {
  ...
  async function readHeicDimensions(file) {
    ...
    const bytes2 = new Uint8Array(await file.slice(0, scanSize).arrayBuffer());
    const view = new DataView(bytes2.buffer, bytes2.byteOffset, bytes2.byteLength);
    let largest = null;
    const candidates = [];
    for (let offset = 4; offset + 16 <= bytes2.length; offset += 1) {
      if (bytes2[offset] !== 105 || bytes2[offset + 1] !== 115 || bytes2[offset + 2] !== 112 || bytes2[offset + 3] !== 101) {
        continue;
      }
      const boxSize = view.getUint32(offset - 4, false);
      if (boxSize < 20 || offset - 4 + boxSize > bytes2.length) continue;
  ...
  function isLikelyMemoryError(error) {
    ...
    return error instanceof RangeError || /memory|allocation|array buffer|out of bounds/.test(message2);
  }
  async function convertHeicToLosslessPng(file, { maxPixels, maxBytes, maxSizeMb } = {}) {
    ...
    const pixelLimit = Number.isFinite(configuredMaxPixels) && configuredMaxPixels > 0 ? configuredMaxPixels : 1e8;
    ...
    const dimensionsMatch = !sourceDimensions || sourceCandidates.some((candidate) => dimensions.width === candidate.width && dimensions.height === candidate.height || dimensions.width === candidate.height && dimensions.height === candidate.width);
    if (!dimensionsMatch) {
      throw createHeicError(
  ...
  function safeUploadFilename(file, role) {
    ...
    return `${PREFIX}/${role}/${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}/${suffix}.${extension}`;
  }
